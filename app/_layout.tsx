...
export default function SignupScreen() {
  ...
  referralCode: referralCode.toUpperCase().trim(),
      ...
      // ✅ AuthContext-কে জানিয়ে দেওয়া হচ্ছে যে ইউজার এখন লগইন করা আছে —
      // এতে status-চেক (পোলিং) সাথে সাথে চালু হয়ে যাবে।
      if (response.token && response.user) {
        setSession(response.token, response.user);
      }

      // ✅✅✅ মূল ফিক্স ✅✅✅
      // আগে এখানে Alert-এর বাটনে চাপলে ম্যানুয়ালি router.replace('/initial-payment')
      // কল করা হতো। কিন্তু setSession() কল হওয়ার সাথে সাথেই app/_layout.tsx-এর
      // RouteGuard নিজে থেকেই (status === 'pending' দেখে) /initial-payment-এ
      // পাঠিয়ে দেয়। দুইটা নেভিগেশন প্রায় একই সময়ে সংঘর্ষ করে পেমেন্ট স্ক্রিন
      // মাঝপথে রিসেট হয়ে যাচ্ছিল — এই কারণেই "not logged in" এরর আসতো, বা
      // পেমেন্ট পেইজ পর্যন্ত পৌঁছাতোই না। এখন এখানে আর ম্যানুয়াল নেভিগেশন
      // করা হচ্ছে না — RouteGuard-ই একমাত্র জায়গা যেটা রাউটিং ঠিক করে।
      ...
      Alert.alert(
        ...
        alertMsg,
        [{ text: 'ঠিক আছে' }] // ✅ কোনো ম্যানুয়াল নেভিগেশন নেই — RouteGuard নিজেই নিয়ে যাবে
      );
